...
def create_tokenizer(tokenizer_type):
  ...
  if tokenizer_type.startswith('bert'):
    ...
    we_filepath = '/youtu_pedestrian_detection/wenzhewang/mmt_data/word_embeddings/word2vec/GoogleNews-vectors-negative300.bin'
    tokenizer = WeTokenizer(we_filepath, freeze=freeze)
  elif tokenizer_type.startswith('grvl'):
    we_filepath = '/youtu_pedestrian_detection/wenzhewang/mmt_data/word_embeddings/GrOVLE/mt_grovle.txt'
    tokenizer = WeTokenizer(we_filepath, freeze=freeze)
  else:
    tokenizer = None
